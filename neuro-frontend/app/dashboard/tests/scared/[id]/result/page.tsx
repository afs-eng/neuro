      ...
      <div className="mx-auto max-w-7xl rounded-[36px] bg-[#f3f0e4] p-5 shadow-2xl ring-1 ring-black/5 md:p-7">
        <div className="rounded-[28px] bg-gradient-to-r from-[#f6f4ed] via-[#f2efe4] to-[#efe7bf] p-5 md:p-6">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-medium tracking-tight text-zinc-900">SCARED - Resultado</h1>
              <p className="mt-1 text-sm text-zinc-600">
                {result.patient_name} • Aplicado em {formatAppliedOn(result.applied_on)}
              </p>
            </div>
            <div className="flex gap-3">
              <Link href={`/dashboard/tests/scared/${params.id}?evaluation_id=${result.evaluation_id}&edit=true`} className="rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-medium text-zinc-700 shadow-sm hover:bg-zinc-50">
                Editar
              </Link>
          ...
          <div className="rounded-[28px] bg-white/70 p-5 shadow-lg ring-1 ring-black/5 mb-6">
            ...
            <p className="text-sm text-slate-600 mb-4">Escala: 0 = Nunca ou raramente verdadeiro | 1 = Algumas vezes verdadeiro | 2 = Bastante ou frequentemente verdadeiro</p>
          ...
          <div className="rounded-[28px] bg-white/70 p-5 shadow-lg ring-1 ring-black/5 mb-6">
            <h3 className="mb-4 text-lg font-semibold text-zinc-900">ITENS DO PROTOCOLO</h3>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {Object.entries(itemLabels).map(([itemNumber, label]) => (
                <div key={itemNumber} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Item {String(itemNumber).padStart(2, "0")}</p>
                  <p className="mt-1 text-sm leading-6 text-zinc-700">{label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[28px] bg-white/70 p-5 shadow-lg ring-1 ring-black/5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-zinc-600">Síntese técnica do SCARED por fator, escore total e leitura integrada do protocolo.</p>
              </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4">
              <p className="text-sm text-amber-800"><strong>Formulário:</strong> {formLabel}</p>
