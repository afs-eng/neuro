from __future__ import annotations


def build_bai_interpretation(computed_payload: dict, patient_name: str | None = None) -> str:
    """Gera texto interpretativo técnico do BAI."""
    name = patient_name or "O examinando"
    faixa = computed_payload["faixa_normativa"]
    interp = computed_payload["interpretacao_faixa"].lower()
    total = computed_payload["total_raw_score"]
    t_score = computed_payload.get("t_score")
    percentile = computed_payload.get("percentile")

    fragments = [
        f"{name} apresentou escore bruto total de {total} no Inventário de Ansiedade de Beck (BAI), "
        f"classificado na faixa {faixa.lower()}, compatível com {interp}."
    ]

    if t_score is not None:
        fragments.append(f" O escore T estimado foi {t_score:.0f}.")

    if percentile is not None:
        fragments.append(f" Esse desempenho corresponde aproximadamente ao percentil {percentile:.1f}.")

    fragments.append(
        " Em análise clínica, o resultado sugere intensidade ansiosa "
        "que deve ser interpretada em conjunto com a entrevista, observações comportamentais "
        "e demais instrumentos do protocolo avaliativo."
    )

    return "".join(fragments)


def interpret_result(classified: dict) -> str:
    """Gera interpretação para uso via API."""
    escore = classified.get("escore_total", 0)
    classificacao = classified.get("classificacao", {})
    nivel = classificacao.get("nivel", "")
    label = classificacao.get("label", "")

    texto = (
        f"O Inventário de Ansiedade de Beck (BAI) apresentou escore total de {escore} pontos, "
        f"o que corresponde a uma classificação de {label}."
    )

    return texto


def get_report_interpretation(merged_data: dict) -> str:
    """Gera parágrafo de interpretação para o relatório clínico."""
    total = merged_data.get("total_raw_score", 0)
    faixa = merged_data.get("faixa_normativa", "")
    interpretacao = merged_data.get("interpretacao_faixa", "").lower()
    t_score = merged_data.get("t_score")
    percentile = merged_data.get("percentile")

    # Busca texto de interpretação existente ou gera novo
    if merged_data.get("interpretation_text"):
        return merged_data["interpretation_text"]

    nome = "O examinando"
    fragments = [
        f"{nome} apresentou escore bruto total de {total} no Inventário de Ansiedade de Beck (BAI), "
        f"classificado na faixa {faixa.lower()}, compatível com {interpretacao}."
    ]

    if t_score is not None:
        fragments.append(f" O escore T estimado foi {t_score:.0f}.")

    if percentile is not None:
        fragments.append(f" Percentil estimado de {percentile:.1f}.")

    fragments.append(
        " Em análise clínica, o resultado sugere intensidade ansiosa "
        "que deve ser interpretada em conjunto com a entrevista, observações comportamentais "
        "e demais instrumentos do protocolo avaliativo."
    )

    return "".join(fragments)
